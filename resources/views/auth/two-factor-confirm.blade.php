...
<body class="antialiased bg-gradient-to-br from-emerald-100/60 via-slate-100 to-teal-100/40 dark:from-[#030906] dark:via-[#06120D] dark:to-[#040C09] text-slate-800 dark:text-slate-100 selection:bg-emerald-500 selection:text-white min-h-screen flex flex-col justify-between p-4 sm:p-6 transition-colors duration-300 relative isolate overflow-x-hidden"
    ...
    <div class="w-full max-w-md mx-auto my-auto opacity-0 animate-fade-in-up relative z-10 py-4">
        ...
        <div class="flex flex-col items-center mb-6 text-center">
            <div class="w-14 h-14 bg-white/70 dark:bg-emerald-500/15 rounded-2xl flex items-center justify-center shadow-sm border border-white/80 dark:border-emerald-400/30 mb-3 backdrop-blur-md">
        ...
        <div class="relative rounded-3xl p-7 sm:p-9 bg-white/55 dark:bg-[#0B1713]/60 border border-white/80 dark:border-emerald-500/25 backdrop-blur-2xl shadow-[0_10px_30px_-5px_rgba(16,185,129,0.1)] dark:shadow-[0_16px_40px_-10px_rgba(0,0,0,0.6)] overflow-hidden">
            ...
            <div class="mb-6 text-center">
                ...
                <h2 class="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white mb-1.5 drop-shadow-sm">Security Verification</h2>
            ...
            <form method="POST" action="{{ route('2fa.confirm') }}" class="space-y-4">
                ...
                <div>
                    <button type="submit" class="w-full bg-emerald-600 hover:bg-emerald-500 active:bg-emerald-700 text-white font-mono font-bold text-xs uppercase tracking-wider py-3.5 rounded-xl transition-all shadow-[0_4px_20px_rgba(16,185,129,0.25)] hover:shadow-[0_6px_24px_rgba(16,185,129,0.35)]">
                        Verify Code & Continue
                    </button>
                </div>
            </form>

            <div class="mt-6 pt-5 border-t border-white/60 dark:border-white/10 text-center">
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="text-xs font-mono font-bold text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-200 transition-colors uppercase tracking-wider">
