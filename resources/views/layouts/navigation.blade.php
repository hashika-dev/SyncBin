<nav x-data="{
...
}" @click.outside="open = false" class="sticky top-0 z-50 w-full bg-white/90 dark:bg-[#0E131F]/90 backdrop-blur-md border-b border-slate-200 dark:border-[#1A2234] transition-colors duration-200">
    <div class="max-w-[1720px] mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-14">
            ...
            <div class="flex items-center gap-1 sm:gap-2 lg:gap-6">
                ...
                <div class="hidden lg:flex items-center gap-1.5 ml-2">
                    <a href="{{ route('dashboard') }}" 
                       class="px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all duration-150 {{ request()->routeIs('dashboard') ? 'bg-slate-100 text-slate-900 border border-slate-300 dark:bg-[#1C2638] dark:text-white dark:border-[#2B3A52] shadow-sm' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-slate-200 dark:hover:bg-[#151D2C]' }}">
                        Live Monitor
                    </a>

                    <a href="{{ route('dashboard.reports') }}" 
                       class="px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all duration-150 {{ request()->routeIs('dashboard.reports') ? 'bg-slate-100 text-slate-900 border border-slate-300 dark:bg-[#1C2638] dark:text-white dark:border-[#2B3A52] shadow-sm' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-slate-200 dark:hover:bg-[#151D2C]' }}">
                        Analytics
                    </a>

                    <a href="{{ route('dashboard.history') }}" 
                       class="px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all duration-150 {{ request()->routeIs('dashboard.history') ? 'bg-slate-100 text-slate-900 border border-slate-300 dark:bg-[#1C2638] dark:text-white dark:border-[#2B3A52] shadow-sm' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-slate-200 dark:hover:bg-[#151D2C]' }}">
                        Audit Trail
                    </a>
                    ...
                    <a href="{{ route('profile.edit') }}" 
                       class="px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all duration-150 {{ request()->routeIs('profile.edit') ? 'bg-slate-100 text-slate-900 border border-slate-300 dark:bg-[#1C2638] dark:text-white dark:border-[#2B3A52] shadow-sm' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-slate-200 dark:hover:bg-[#151D2C]' }}">
                        Settings
                    </a>
                </div>
            </div>
            ...
            <div class="hidden sm:flex items-center gap-3.5">
                ...
                <!-- Theme Toggle Button -->
                <button @click="toggleTheme()" type="button" class="p-1.5 rounded-lg text-slate-500 hover:text-slate-900 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-slate-200 dark:hover:bg-[#151D2C] transition-colors" title="Toggle Theme" aria-label="Toggle Color Theme">
            ...
            <div class="flex items-center gap-2 lg:hidden">
                <button @click="open = !open" 
                        class="p-2 rounded-lg text-slate-500 hover:text-slate-900 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-white dark:hover:bg-[#151D2C] focus:outline-none focus:ring-2 focus:ring-emerald-500/30 transition-colors" 
    ...
    <div x-show="open" 
         ...
         class="lg:hidden border-t border-slate-200 dark:border-[#1A2234] bg-white/95 dark:bg-[#0E131F]/95 backdrop-blur-lg px-4 pt-3 pb-5 space-y-2 shadow-xl"
         x-cloak>
        
        <!-- Mobile User Info & Role Banner -->
        <div class="flex items-center justify-between pb-3 mb-2 border-b border-slate-200 dark:border-[#1C2638]">
            ...
            <!-- Mobile Theme Toggle -->
            <button @click="toggleTheme()" type="button" class="p-2 rounded-lg bg-slate-100 dark:bg-[#161D2B] border border-slate-200 dark:border-[#243046] text-slate-700 dark:text-slate-300 text-xs flex items-center gap-1.5 shrink-0" title="Toggle Theme">
        ...
        <a href="{{ route('dashboard') }}" @click="open = false" 
           class="flex items-center justify-between px-3.5 py-2.5 rounded-xl text-xs font-semibold transition-colors {{ request()->routeIs('dashboard') ? 'bg-slate-100 text-slate-900 border border-slate-300 dark:bg-[#1C2638] dark:text-white dark:border-[#2B3A52]' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-white dark:hover:bg-[#151D2C]' }}">
