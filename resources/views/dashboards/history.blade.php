<x-app-layout>
    ...
    <div class="flex min-h-screen bg-gradient-to-br from-rose-50 via-white to-orange-50/50 dark:from-zinc-950 dark:via-zinc-900 dark:to-zinc-950 text-gray-900 dark:text-zinc-100 transition-colors duration-300" x-data="{ showClearConfirm: false }">
        <!-- Sidebar -->
        ...
        <main class="flex-1 ml-72 p-16">
            ...
            <div class="bg-white/70 dark:bg-zinc-900/70 backdrop-blur-xl rounded-[2.5rem] shadow-2xl dark:shadow-none border border-white dark:border-zinc-800 p-8 mb-12 transition-colors duration-300">
                <form action="{{ route('dashboard.history') }}" method="GET" class="flex flex-col md:flex-row gap-6 items-end">
                    <div class="flex-1 w-full">
                        <label for="search" class="block text-xs font-black uppercase tracking-widest text-rose-400 dark:text-zinc-500 mb-3">Search Items</label>
                        <input type="text" name="search" id="search" placeholder="Type item name (e.g. Battery)..." value="{{ request('search') }}"
                    ...
                    <div class="w-full md:w-64">
                        <label for="bin" class="block text-xs font-black uppercase tracking-widest text-rose-400 dark:text-zinc-500 mb-3">Classification Filter</label>
                    ...
                    <div class="flex gap-4 w-full md:w-auto">
                        <button type="submit" class="flex-1 md:flex-initial px-8 py-4 bg-rose-500 hover:bg-rose-600 text-white rounded-2xl font-black shadow-lg shadow-rose-200 transition-all text-sm uppercase tracking-wider">
                            Filter
                        </button>
                        @if(request()->anyFilled(['search', 'bin']))
                            <a href="{{ route('dashboard.history') }}" class="flex-1 md:flex-initial px-8 py-4 bg-gray-100 dark:bg-zinc-800 hover:bg-gray-200 dark:hover:bg-zinc-700 text-gray-700 dark:text-zinc-300 rounded-2xl font-black transition-all text-sm text-center uppercase tracking-wider">
                                Reset
                            </a>
                        @endif
                    </div>
